...
const Header = () => {
  ...
  return (
    <header className='p-2 py-4 flex justify-between items-center'>
      <div className='flex justify-between gap-20'>
        ...
        <nav className='hidden md:block'>
          <Menu className="flex gap-4" linkStyle="block w-full px-2 py-1.5 hover:bg-gray-100 mt-1 text-slate-600" />
        </nav>
      </div>
      <div className='md:hidden relative'>
        ...
        {isMenuOpen && (
          <div ref={menuRef} className='absolute -start-36 pt-2 shadow-lg bg-[#fcfafa]'>
            <Menu className="flex flex-col gap-4 w-50" linkStyle="block w-full px-4 py-1.5 hover:bg-gray-200 text-slate-600" />
          </div>
        )}
      </div>
...
const Menu: FC<any> = ({ className, linkStyle }) => {
  return (
